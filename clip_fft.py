...
from lucid.utils import slice_imgs, derivat, sim_func, aesthetic_model, basename, img_list, img_read, plot_text, txt_clean, checkout, old_torch
from lucid.image import to_valid_rgb, fft_image, dwt_image
import lpips
...
def get_args():
    ...
    parser.add_argument('-ops', '--opt_step', default=1,
                        type=int, help='How many optimizing steps per save step')
    ...
    parser.add_argument('--dwt',     action='store_true',
                        help='Use DWT instead of FFT')
    parser.add_argument('-w',  '--wave',    default='coif2',
                        help='wavelets: db[1..], coif[1..], haar, dmey')
    # tweaks
    parser.add_argument('-a',  '--align',   default='uniform', choices=[
    ...
    parser.add_argument('-opt', '--optimizer', default='adam_custom',
                        choices=['adam', 'adamw', 'adam_custom', 'adamw_custom'], help='Optimizer')
    ...
    parser.add_argument('-c',  '--sync',    default=0,
                        type=float, help='Sync output to input image')
    parser.add_argument('--invert',  action='store_true',
                        help='Invert criteria')
    parser.add_argument('--sim',     default='mix',
                        help='Similarity function (dot/angular/spherical/mixed; None = cossim)')
    a = parser.parse_args()
    ...
    if len(a.size) == 1:
        a.size = a.size * 2
    if (a.in_img is not None and a.sync != 0) or a.resume is not None:
        a.align = 'overscan'
...
def main():
    ...
    if a.optimizer.lower() == 'adamw':
        ...
        optimizer = torch.optim.AdamW(
            params, lr0, weight_decay=0.01, betas=(.0, .999), amsgrad=True)
        ...
        for subtxt in txt.split('|'):
            ...
            if a.multilang is True:
                emb = model_lang.encode(
                    [subtxt], convert_to_tensor=True, show_progress_bar=False)
            else:
                emb = model_clip.encode_text(clip.tokenize(subtxt))
    ...
    if a.in_img is not None and os.path.isfile(a.in_img):
        ...
        img_in = torch.from_numpy(
            img_read(a.in_img)/255.).unsqueeze(0).permute(0, 3, 1, 2)
        img_in = img_in[:, :3, :, :]  # fix rgb channels
        in_sliced = slice_imgs([img_in], a.samples,
                               a.modsize, transforms.normalize(), a.align)[0]
        img_enc = model_clip.encode_image(in_sliced).detach().clone()
        if a.dualmod is not None:
            img_enc2 = model_clip2.encode_image(in_sliced).detach().clone()
        if a.sync > 0:
            sim_loss = lpips.LPIPS(net='vgg', verbose=False)
            sim_size = [s//2 for s in a.size]
            img_in = F.interpolate(
                img_in, sim_size, mode='bicubic', align_corners=True).float()
        else:
            del img_in
        del in_sliced
    ...
    out_name += '-%s' % a.model.replace('/', '').replace(
        '-', '') if a.dualmod is None else '-dm%d' % a.dualmod
    ...
    def train(i):
        ...
        noise = a.noise * \
            torch.rand(1, 1, *params[0].shape[2:4], 1) if a.noise > 0 else None
        img_out = image_f(noise)
        img_sliced = slice_imgs([img_out], a.samples,
                                a.modsize, trform_f, a.align, a.macro)[0]
        ...
        # image composition
        if a.sync > 0 and a.in_img is not None and os.path.isfile(a.in_img):
            prog_sync = (a.steps // a.opt_step - i) / (a.steps // a.opt_step)
            loss += prog_sync * a.sync * sim_loss(F.interpolate(
                img_out, sim_size, mode='bicubic', align_corners=True).float(), img_in, normalize=True).squeeze()
        ...
        if a.enforce != 0:
            img_sliced = slice_imgs(
                [image_f(noise)], a.samples, a.modsize, trform_f, a.align, a.macro)[0]
        ...
        if i % a.opt_step == 0:
            ...
            checkout(img, os.path.join(tempdir, '%04d.jpg' %
                     (i // a.opt_step)), verbose=a.verbose)
            pbar.upd()
    ...
    shutil.copy(img_list(
        tempdir)[-1], os.path.join(a.out_dir, '%s-%d.jpg' % (out_name, a.steps)))
